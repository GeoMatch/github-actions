locals {
  ...
  ssm_name_prod_user_public_key    = "${local.ssm_name_prefix}/SFTP_PROD_USER_PUBLIC_KEY"
  ...
  ssm_name_prod_username           = "${local.ssm_name_prefix}/SFTP_PROD_USERNAME"
  ssm_name_staging_username        = "${local.ssm_name_prefix}/SFTP_STAGING_USERNAME"
}

resource "aws_ssm_parameter" "prod_user_public_key" {
  name        = local.ssm_name_prod_user_public_key
  type        = "SecureString"
  value       = "placeholder"
...
data "aws_ssm_parameter" "prod_user_public_key" {
  name = local.ssm_name_prod_user_public_key
  depends_on = [
...
resource "aws_ssm_parameter" "prod_sftp_username" {
  name        = local.ssm_name_prod_username
  type        = "SecureString"
  value       = "prod-username"
  description = ""
  overwrite   = false
...
data "aws_ssm_parameter" "prod_sftp_username" {
  name = local.ssm_name_prod_username
  depends_on = [
    aws_ssm_parameter.prod_sftp_username
  ]
}
